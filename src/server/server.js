...
import "fs";
import "path";
import "replacestream";
import "send";
import "./auth";
import * as Ipc from "./ipc/server";
import { init } from "./theme";
import "./zeroconf";
import { resolveHomeDir } from "./util";
let server = createServer(auth ? auth.check(httpRoute) : httpRoute);
let ipc = new Ipc(server);
let theme = init();
let prod = !process.argv[0].includes("node_modules");
let debug = settings.read("debug");
let osc = {};
let clients = {};
let httpCheckTimeout;
    ...
    if (url === "/qrcode") {
        ...
        fs.createReadStream(path.resolve(__dirname + "/../client/index.html"))
            ...
            <script>
                ...
                window.KIOSK=${
                    settings.read("read-only") && settings.read("fullscreen")
                        ? "true"
                        : "false"
                }
                window.AUTH=\`${(req.headers.authorization || "")
                    .replace("Basic ", "")
                    .replace("/", "_")}\`
            </script></body>`
                )
